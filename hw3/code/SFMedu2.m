...
% frames.images{1}='images/temple0001.png';
% frames.images{2}='images/temple0002.png';
% frames.images{3}='images/temple0003.png';
% frames.images{4}='images/temple0004.png';
% frames.images{5}='images/temple0005.png';

% frames.images{1}='images/viff.000.ppm';
% frames.images{2}='images/viff.001.ppm';
% frames.images{3}='images/viff.002.ppm';
% frames.images{4}='images/viff.003.ppm';
% frames.images{5}='images/viff.004.ppm';
...
for frame=1:frames.length-1    
    ...
    printReprojectionError(Graph{frame});
end
...
% merge according to keypoint correspondences
num_match = zeros(1, length(Graph));
for c=1:length(Graph)
    num_match(c) = size(Graph{c}.matches, 2);
end
[~, mergeOrder] = sort(num_match, 'descend');
%Graph = Graph(mergeOrder);
...
for frame=2:frames.length-1
    ...
    rpe_before = [rpe_before, printReprojectionError(mergedGraph)];
    % bundle adjustment
    mergedGraph = bundleAdjustment(mergedGraph);    
    
    if visualize, visualizeGraph(mergedGraph,frames); title('after bundle adjustment'); end
    
    rpe_after = [rpe_after, printReprojectionError(mergedGraph)];
end
...
% figure; hold on;
% plot([2 3 4], rpe_before); plot([2 3 4], rpe_after);
...
if frames.focal_length ~= mergedGraph.f
    ...
    %frames.K = f2K(frames.focal_length);
    frames.K = [mergedGraph.f, 0, mergedGraph.px; 0, mergedGraph.f, mergedGraph.py; 0 0 1];
    disp(frames.K);
end
