function [ vps ] = findVPs( im )
% findVPs: finds 3 vanishing points assuming 3 orthogonal directions

% based on the work of:
% Bo Li, Kun Peng, Xianghua Ying, Hongbin Zha. 
% Simultaneous Vanishing Point Detection and Camera Calibration from Single Images. ISVC 2010

f = rgb2gray(im);
line = APPgetLargeConnectedEdges(f, 40);
...
vanishing = [foc*d1*n1'; foc*d2*n2'; foc*d3*n3'];


v1 = [(vanishing(1,1)+size(f,2)/2), (vanishing(1,2)+size(f,1)/2), 1];
v2 = [(vanishing(2,1)+size(f,2)/2), (vanishing(2,2)+size(f,1)/2), 1];
v3 = [(vanishing(3,1)+size(f,2)/2), (vanishing(3,2)+size(f,1)/2), 1];
vps = [v1; v2; v3];
